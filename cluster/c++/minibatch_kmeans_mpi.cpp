...
MiniBatchKMeansMPI::MiniBatchKMeansMPI(int K, int batch_size, int max_iter,double tol)
    : K(K), batch_size(batch_size), max_iter(max_iter), tol(tol),initialized(false) {}
...
void MiniBatchKMeansMPI::partial_fit(std::vector<std::vector<double>> &local_X)
{
    ...
    //Step 1:Rank 0 initialize centroids

    if (!initialized) {
        update_count.resize(K, 0);
        if (rank == 0) {
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        MPI_Allreduce(local_sum.data(), global_sum.data(), K * dim, MPI_DOUBLE,
                      MPI_SUM, MPI_COMM_WORLD);

        MPI_Allreduce(local_count.data(), global_count.data(), K, MPI_INT,
                      MPI_SUM, MPI_COMM_WORLD);

        //Update centroid
        for (int k = 0; k < K; k++) {
            if (global_count[k] > 0) {
                int old_count = update_count[k];
                update_count[k] += global_count[k];

                if (old_count == 0) {
                    for (int d = 0; d < dim; d++) {
                        centroids[k][d] =
                            global_sum[k * dim + d] / global_count[k];
                    }
                } else {
                    for (int d = 0; d < dim; d++) {
                        centroids[k][d] = (centroids[k][d] * old_count +global_sum[k * dim + d]) /update_count[k];
                    }
                }
            }
        }
