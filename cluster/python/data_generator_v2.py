...
class DataDrivenGenerator:
    ...
    def prepare_statistics(self):
        ...
        for product, group in self.df.groupby("product_id"):
            ...
            self.product_profile[product] = {
                ...
                "review_mean":
                    group["review_count"].mean(),

                "stock_mean":
                    group["stock"].mean(),

                "seller_rating_mean":
                    group["seller_rating"].mean(),
    ...
    def generate_new_user(self):
        ...
        purchase_count = np.random.choice(
            self.user_behavior_model["purchase_count_values"]
        )

        spending_level = np.random.choice(
            self.user_behavior_model["spending_level_values"]
        )
        ...
        return {
            ...
            "category_values":
                list(category_preference.keys()),

            "category_probs":
                list(category_preference.values())
        }
